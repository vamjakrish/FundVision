...
import ProofOfTrustSection from '../components/campaigns/ProofOfTrustSection';
        ...
        <div className="grid lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-5 sm:space-y-6 min-w-0">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                {campaign.verificationStatus === 'verified' && (
                  <span className="badge bg-green-50 text-green-700 border border-green-200 text-xs flex items-center gap-1 font-semibold">
            ...
            <div className="border-b border-slate-200 overflow-x-auto no-scrollbar">
              <div className="flex gap-4 sm:gap-6 min-w-max">
                {[
                  ...
                  { id: 'transparency', label: 'Proof & Transparency' },
                  { id: 'updates', label: `Updates (${campaign.updates?.length || 0})` },
                  { id: 'donors', label: `Donors (${campaign.donorCount || 0})` }
                ].map(t => (
                  <button key={t.id} onClick={() => setActiveTab(t.id)}
                    className={`pb-3 text-sm font-semibold whitespace-nowrap border-b-2 transition-colors ${activeTab === t.id ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                    {t.label}
                  </button>
                ))}
              </div>
            ...
            {activeTab === 'transparency' && (
              <ProofOfTrustSection campaignId={campaign._id} campaign={campaign} />
            )}
            {activeTab === 'updates' && (
              <div className="space-y-4">
          ...
          <div className="hidden lg:flex flex-col gap-5 min-w-0">
            ...
            {/* Proof of Trust Transparency Widget */}
            <div className="card p-5 space-y-3">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-primary" />
                  <h3 className="font-semibold text-slate-800 text-sm">Proof of Trust</h3>
                </div>
                <span className={`badge text-[11px] font-bold ${
                  ...
                  {campaign.verificationStatus === 'verified' ? 'Verified' : 'In Review'}
                </span>
              </div>
              <p className="text-xs text-slate-500 leading-relaxed">
                {campaign.verificationStatus === 'verified'
                  ? 'This campaign has completed the platform Proof of Trust review with verified supporting documents.'
                  : 'Supporting proof documents and fund plan submitted for platform review.'}
              </p>
              <button
                type="button"
              ...
              >
                View Verified Evidence & Fund Plan →
              </button>
            </div>

...
import { ArrowLeft, Upload, X, Info, PieChart, ShieldCheck } from 'lucide-react';
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        fundUtilization: fundUtilization.length > 0 ? fundUtilization : undefined,
      };
      delete payload.customCategory;
      const res = await campaignAPI.create(payload);
      toast.success('Campaign created successfully! 🎉');
      toast.success('You can now upload supporting Proof of Trust documents from your dashboard.');
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="pt-20 min-h-screen bg-slate-50 pb-20">
      <div className="section-container py-8 max-w-3xl">
        ...
        <div className="flex items-center gap-2 mb-8">
          {['Basic Info', 'Story', 'Fund Plan', 'Images'].map((label, i) => (
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {/* Step 3: Fund Utilization Plan */}
          {step === 3 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="card p-6 sm:p-8 space-y-6">
              <div className="p-4 bg-primary-50 dark:bg-primary-950/30 rounded-2xl border border-primary-100 dark:border-primary-900/30 flex items-start gap-3">
                <ShieldCheck className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                <div className="text-xs sm:text-sm text-slate-700 dark:text-slate-200">
                  <span className="font-bold">Transparent Fund Utilization Plan</span>
                  <p className="mt-0.5 text-slate-600 dark:text-slate-300">
                    Campaigns with clear, itemized fund allocation earn higher donor trust and faster verification approval.
                  </p>
                </div>
              </div>

              <FundUtilizationEditor
                items={fundUtilization}
                onChange={setFundUtilization}
                goalAmount={Number(goalAmount) || 0}
              />

              <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                <button type="button" onClick={() => setStep(2)} className="btn-secondary flex-1 py-3">← Back</button>
                <button type="button" onClick={() => setStep(4)} className="btn-primary flex-1 py-3">Continue to Images →</button>
              </div>
            </motion.div>
          )}
              ...
              <div className="card p-5 sm:p-6">
                ...
                <div className="space-y-2 text-sm">
                  {[
                    ...
                    ['Planned Utilization', `₹${fundUtilization.reduce((sum, i) => sum + (Number(i.estimatedAmount) || 0), 0).toLocaleString()} (${fundUtilization.length} items)`],
                    ['Deadline', watch('deadline')],
                  ].map(([k, v]) => v && (
                    <div key={k} className="flex gap-2 flex-wrap">
                      <span className="text-slate-400 w-32 shrink-0 text-xs sm:text-sm">{k}:</span>
                ...
                <div className="mt-4 p-3 bg-amber-50 rounded-xl border border-amber-200 text-sm text-amber-700">
                  ⏳ After creation, you can upload official Proof of Trust documents (hospital estimates, bills, etc.) to get your campaign verified.
                </div>
              </div>

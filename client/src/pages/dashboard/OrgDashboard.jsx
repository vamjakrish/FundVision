...
import { Plus, TrendingUp, Users, Target, Edit, Eye, CheckCircle, Clock, BadgeCheck, BarChart2, Sparkles, ShieldCheck, Megaphone, X, Upload, AlertTriangle, FileText } from 'lucide-react';
...
const VERIFY_STATUS_CONFIG = {
  verified: { tone: 'success', label: 'Verified Proof' },
  more_info_required: { tone: 'warning', label: 'Action Required' },
  pending: { tone: 'warning', label: 'Verification Pending' },
  under_review: { tone: 'primary', label: 'Under Review' },
  rejected: { tone: 'danger', label: 'Proof Rejected' },
  suspended: { tone: 'danger', label: 'Suspended' },
  not_submitted: { tone: 'slate', label: 'Proof Not Submitted' },
};
  ...
  return (
    <div className="pt-20 min-h-screen bg-slate-50">
      <div className="section-container py-5 sm:py-8">
        ...
        {/* Actionable Alert Banner for campaigns requiring more info */}
        {campaigns.some(c => c.verificationStatus === 'more_info_required') && (
          <Card className="mb-6 bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 shadow-sm" padding="p-4 sm:p-5">
            <div className="flex items-start justify-between gap-4 flex-wrap sm:flex-nowrap">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-6 h-6 text-amber-600 shrink-0 mt-0.5" />
                <div>
                  <p className="font-bold text-amber-900 dark:text-amber-200 text-sm sm:text-base">
                    Action Required on Campaign Verification
                  </p>
                  <p className="text-xs sm:text-sm text-amber-800 dark:text-amber-300 mt-0.5">
                    Administrators have requested additional proof documents or information for one or more campaigns.
                  </p>
                </div>
              </div>

              {campaigns.find(c => c.verificationStatus === 'more_info_required') && (
                <Link
                  to={`/campaigns/${campaigns.find(c => c.verificationStatus === 'more_info_required')._id}/edit?tab=proofs`}
                  className="shrink-0"
                >
                  <Button variant="primary" size="sm" icon={ShieldCheck} iconPosition="left">
                    View & Respond
                  </Button>
                </Link>
              )}
              ...
              {campaignsLoading ? (
                ...
                <div className="space-y-3">
                  {campaigns.slice(0, 4).map(c => {
                    const vStatus = c.verificationStatus || 'not_submitted';
                    const vConfig = VERIFY_STATUS_CONFIG[vStatus] || VERIFY_STATUS_CONFIG.not_submitted;
                    return (
                      <div key={c._id} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-slate-50 flex-wrap sm:flex-nowrap">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            <Badge tone={vConfig.tone}>{vConfig.label}</Badge>
                            <span className="text-xs text-slate-400">₹{c.raisedAmount?.toLocaleString()} raised</span>
                          </div>
                        </div>
                        <div className="flex gap-2 shrink-0">
                          <Link to={`/campaigns/${c._id}/edit?tab=proofs`} title="Manage Proof of Trust" className="p-2 rounded-lg hover:bg-white text-slate-400 hover:text-primary transition-colors">
                    ...
                    <motion.div key={c._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-4 text-xs text-slate-500 flex-wrap">
                          ...
                          <span>{c.fundUtilization?.length ? `${c.fundUtilization.length} plan items` : 'No fund plan'}</span>
                      ...
                      <div className="flex gap-2 shrink-0 w-full sm:w-auto flex-wrap sm:flex-nowrap">
                        <Link to={`/campaigns/${c._id}/edit?tab=proofs`} className="flex-1 sm:flex-none">
                          <Button variant="secondary" size="sm" icon={ShieldCheck} iconPosition="left" fullWidth>
                            Proofs
                          </Button>
                        </Link>

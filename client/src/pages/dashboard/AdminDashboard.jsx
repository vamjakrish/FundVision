...
import AdminProofVerificationQueue from '../../components/admin/AdminProofVerificationQueue';
...
function AdminDashboard() {
  ...
  const { data: vQueueRes } = useQuery({ queryKey: ['admin-verification-queue'], queryFn: () => adminAPI.getVerificationQueue().then(r => r.data) });
  ...
  const pendingVerifications = vQueueRes?.data || [];
  ...
  const TABS = ['overview', 'verifications', 'organizations', 'campaigns', 'users', 'blockchain', 'analytics'];
    ...
    <div className="pt-20 min-h-screen bg-slate-50">
      <div className="section-container py-5 sm:py-8">
        <DashboardHeader
          ...
          subtitle="Manage NGOs, campaigns, proof of trust verifications, users, and blockchain integrity across FundVision."
          badges={[
            pendingVerifications.length > 0 && <Badge key="v" tone="primary" icon={ShieldCheck}>{pendingVerifications.length} proofs pending</Badge>,
            pendingOrgs.length > 0 && <Badge key="o" tone="warning" icon={AlertTriangle}>{pendingOrgs.length} orgs pending</Badge>,
            pendingCamps.length > 0 && <Badge key="c" tone="slate" icon={Flag}>{pendingCamps.length} campaigns pending</Badge>,
        ...
        <DashboardTabs
          ...
          counts={{ verifications: pendingVerifications.length, organizations: pendingOrgs.length, campaigns: pendingCamps.length, blockchain: bcStats?.totalFailed || 0 }}
        />
        ...
        {/* Proof of Trust Verification Queue */}
        {activeTab === 'verifications' && (
          <AdminProofVerificationQueue />
        )}

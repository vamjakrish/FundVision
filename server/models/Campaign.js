...
const campaignSchema = new mongoose.Schema({
  ...
  verificationStatus: {
    type: String,
    enum: ['not_submitted', 'pending', 'under_review', 'more_info_required', 'verified', 'rejected', 'suspended'],
  ...
  fundUtilization: [{
    purpose: {
      type: String,
      required: [true, 'Fund utilization purpose is required'],
      trim: true,
    ...
    estimatedAmount: {
      ...
      min: [1, 'Estimated amount must be greater than zero']
    },
    description: {
      type: String,
      trim: true,
...
campaignSchema.index({ verificationStatus: 1 });
campaignSchema.index({ createdAt: -1 });

// Virtual: total utilization
campaignSchema.virtual('totalUtilization').get(function() {
  if (!this.fundUtilization || !this.fundUtilization.length) return 0;
  return this.fundUtilization.reduce((sum, item) => sum + (Number(item.estimatedAmount) || 0), 0);
});

...
// @desc    Get campaigns for proof verification queue
// @route   GET /api/admin/campaigns/verification-queue
const getVerificationQueue = async (req, res, next) => {
  try {
    ...
    if (status && status !== 'all') {
      ...
      query.verificationStatus = { $in: ['pending', 'under_review', 'more_info_required'] };
    }
    ...
    const campaigns = await Campaign.find(query)
      .populate('organization', 'name logo isVerified type address phone')

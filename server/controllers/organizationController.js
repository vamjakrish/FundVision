...
// @desc    Get organization by ID (public / sanitized)
...
const getOrganization = async (req, res, next) => {
  try {
    ...
    // Authorization check: only the organization owner or admin can view confidential documents & PAN
    const isOwner = req.user && org.user && (
      (org.user._id && req.user.id.toString() === org.user._id.toString()) ||
      (req.user.id.toString() === org.user.toString())
    );
    const isAdmin = req.user && req.user.role === 'admin';

    if (!isOwner && !isAdmin) {
      const sanitized = org.toObject ? org.toObject() : { ...org };
...
// @desc    Get all organizations (public / sanitized)
...
const getOrganizations = async (req, res, next) => {
  try {
    ...
    const [orgs, total] = await Promise.all([
      Organization.find(query).select('-documents -panNumber').sort('-totalRaised').skip(skip).limit(parseInt(limit)),
